document.addEventListener("DOMContentLoaded", () => {
  ...
  const communeNameInput = document.getElementById("commune-name");
  ...
  function fetchAndDisplayWeather() {
    ...
    if (!commune || selectedDays.length === 0) return;

    resultsContainer.innerHTML = "Chargement des données météo...";

    fetch(`https://api.meteo-concept.com/api/forecast/daily?token=f7d06d2b23546fe98fcab761fda383ba1fc4c9be665e3f1a0e2e9d238538a18a&insee=${commune}&fields=uv,tmin,tmax,rr10,wind10m,dirwind10m,humidity,altitude,tmin_feel,tmax_feel`)
      .then((response) => response.json())
      .then((data) => {
        resultsContainer.innerHTML = "";
        selectedDays.forEach((day) => {
          const dataDay = data.forecast[parseInt(day) - 1];
          const card = createWeatherCard(dataDay, options, data.city, data.city.name);
  ...
  async function updateCommuneList(communes) {
    communeSelect.innerHTML = `<option value="">Sélectionner une commune</option>`;
    communes.sort((a, b) => a.nom.localeCompare(b.nom)).forEach(commune => {
      const option = document.createElement("option");
      option.value = commune.code;
      option.textContent = `${commune.nom} (${commune.codesPostaux[0]})`;
  ...
  zipcodeInput.addEventListener("input", async (e) => {
    ...
    if (/^\d{5}$/.test(code)) {
      try {
        const response = await fetch(`https://geo.api.gouv.fr/communes?codePostal=${code}&fields=nom,codesPostaux,code`);
        const data = await response.json();
        updateCommuneList(data);
      } catch (err) {
  ...
  communeNameInput.addEventListener("input", async (e) => {
    const name = e.target.value.trim();
    if (name.length < 2) return;
    communeSelect.innerHTML = `<option value="">Chargement...</option>`;

    try {
      const response = await fetch(`https://geo.api.gouv.fr/communes?nom=${name}&fields=nom,codesPostaux,code`);
      const data = await response.json();
      updateCommuneList(data);
    } catch (err) {
      communeSelect.innerHTML = `<option value="">Erreur de chargement</option>`;
  ...
  function createWeatherCard(data, options, city, communeName) {
    ...
    if (options.includes("humidity") && data.humidity !== undefined) {
      card.appendChild(createLine("Humidité", `${data.humidity} %`));
    }
    if (options.includes("feelslike")) {
      card.appendChild(createLine("Temp. ressentie min", `${data.tmin_feel}°C`));
      card.appendChild(createLine("Temp. ressentie max", `${data.tmax_feel}°C`));
    }
    if (options.includes("altitude") && city.altitude) {
      card.appendChild(createLine("Altitude", `${city.altitude} m`));
    }
